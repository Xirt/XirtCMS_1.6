<div id='dvConfig' style='display: none;'>
	...
	<fieldset class='box-form'>
		...
		{html_options name=x_search_type options=$xLang->options['search'] selected=$xConf->search_type id='toggler'}
		...
		<input type='text' name='x_default_value' value="{$xConf->default_value}" />

		<br />

		<label for='x_default_limit'>{$xLang->labels['defaultLimit']}</label>
		<input type='text' name='x_default_limit' value="{$xConf->default_limit}" class='required validate-digits' />

		<br />

		<label>{$xLang->labels['defaultMethod']}</label>
		<div id='method_0'>{html_options name=x_default_method_0 options=$xLang->options['normal'] selected=$xConf->default_method}</div>
		<div id='method_1'>{html_options name=x_default_method_1 options=$xLang->options['fulltext'] selected=$xConf->default_method}</div>

		<label for='x_titlelength'>{$xLang->labels['titleLength']}</label>
		<input type='text' name='x_titlelength' value="{$xConf->titlelength}" class='required validate-digits' />

		<br />

		<label for='x_textlength'>{$xLang->labels['textLength']}</label>
		<input type='text' name='x_textlength' value="{$xConf->textlength}" class='required validate-digits' />

		<br />

		<label for='x_node_id'>{$xLang->labels['nodeId']}</label>
		<input type='text' name='x_node_id' value="{$xConf->node_id}" class='required validate-digits' />
